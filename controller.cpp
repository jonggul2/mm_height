...
float findClosestY(const vector<float> terrain_y, float position_x, float position_z) {
    ...
    int index_x = static_cast<int>((position_x + 10) * 10);
    if (index_x < 0)
        index_x = 0;
    else if (index_x > 200)
        index_x = 200;

    int index_z = static_cast<int>((position_z + 10) * 10);
    if (index_z < 0)
        index_z = 0;
    else if (index_z > 200)
        index_z = 200;

    int index_y = index_x + index_z * 201;
...
{
    // float run = 0.0f;
    // if (IsKeyPressed(KEY_A)) run = 1.0f;
    // else if (IsKeyPressed(KEY_S)) run = 0.0f;
    ...
    const float radius = 0.6f)
{
    ...
    for (int j = 0; j < substeps; j++)
    {
        ...
        // pair<float, float> point = {sqrt(proj_pos.x * proj_pos.x + proj_pos.z * proj_pos.z), proj_pos.y};
        ...
        //     // {
        //     //     if ((nearest.y + obstacles_scales(i).y - prev_pos.y) > 0.5f)
        //     //     {
        //     //         // cout << "proj_pos.x: " << proj_pos.x << ", proj_pos.y: " << proj_pos.y << ", proj_pos.z: " << proj_pos.z << endl;
        //     //         proj_pos = radius * normalize(proj_pos - nearest) + nearest;
        //     //         // cout << "after: proj_pos.x: " << proj_pos.x << ", proj_pos.y: " << proj_pos.y << ", proj_pos.z: " << proj_pos.z << endl;
        //     //     }
        //     // } 


        //     if (obstacles_positions(i).x - 0.5f * obstacles_scales(i).x <= proj_pos.x &&
        //     proj_pos.x <= obstacles_positions(i).x + 0.5f * obstacles_scales(i).x &&
        //     obstacles_positions(i).z - 0.5f * obstacles_scales(i).z <= proj_pos.z &&
        //     proj_pos.z <= obstacles_positions(i).z + 0.5f * obstacles_scales(i).z) 
        //     {
        //         proj_pos.y = maxf(proj_pos.y, obstacles_positions(i).y + 1.0f * obstacles_scales(i).y);
        //     }
        // }
    } 
    ...
    vector<float> terrain_y)
{
    ...
    // // if (IsKeyPressed(KEY_SPACE) && position.y == 0){
    ...
    // for (int i = 0; i < obstacles_positions.size; i++)
    // {
    //     if (obstacles_positions(i).x - 0.5f * obstacles_scales(i).x <= position.x &&
    //     position.x <= obstacles_positions(i).x + 0.5f * obstacles_scales(i).x &&
    //     obstacles_positions(i).z - 0.5f * obstacles_scales(i).z <= position.z &&
    //     position.z <= obstacles_positions(i).z + 0.5f * obstacles_scales(i).z) 
    //     {
    //         t_height = maxf(t_height, obstacles_positions(i).y + 1.0f * obstacles_scales(i).y);
    //     }
    // }

    // if (position.y > t_height) acceleration.y -= gravity;

    // // if (position.y < t_height && position.y > position_prev.y && (contact_states(0) | contact_states(1))) {
    // //     acceleration.y += 10.0f;
    // // }
    ...
    // // for (int i = 0; i < obstacles_positions.size; i++)
    // // {
    // //     if (obstacles_positions(i).x - 0.5f * obstacles_scales(i).xcout << position.y - map_y << endl; <= position.x &&
    // //     position.x <= obstacles_positions(i).x + 0.5f * obstacles_scales(i).x &&
    // //     obstacles_positions(i).z - 0.5f * obstacles_scales(i).z <= position.z &&
    // //     position.z <= obstacles_positions(i).z + 0.5f * obstacles_scales(i).z &&
    // //     position.y <= t_height) 
    ...
    const float eps=1e-8)
{
    ...
    // contact_position.y = findClosestY(terrain_x, terrain_z, terrain_y, contact_position.x, contact_position.z);
...
void draw_features(const slice1d<float> features, const vec3 pos, const quat rot, const Color color)
{
    ...
    // printf("traj0_pos.y: %f \n", traj0_pos.y);



    // cout << "traj0_dir.x:" << traj0_dir.x << " y:" << traj0_dir.y << " z:" << traj0_dir.z << endl;
    ...
    // DrawSphere(to_Vector3(lfoot_pos), 0.1f, color);
    // DrawSphere(to_Vector3(rfoot_pos), 0.1f, color);
    // DrawSphere(to_Vector3(traj0_pos), 0.1f, color);
    // DrawSphere(to_Vector3(traj1_pos), 0.1f, color);
    // DrawSphere(to_Vector3(traj2_pos), 0.1f, color);
    ...
    const Color color)
{
    for (int i = 1; i < trajectory_positions.size; i++)
    {
        ...
        // DrawSphere(to_Vector3(trajectory_positions(i)), 0.1f, color);
...
// Init Window
    ...
    // obstacles_positions(3) = vec3(5.0f, 0.0f, -2.0f);
    // obstacles_positions(4) = vec3(5.0f, 0.2f, -2.0f);
    // obstacles_positions(5) = vec3(5.0f, 0.4f, -2.0f);

    // obstacles_positions(6) = vec3(5.0f, 0.2f, -6.0f);
    // obstacles_positions(7) = vec3(5.0f, 0.4f, -6.0f);
    // obstacles_positions(8) = vec3(5.0f, 0.6f, -6.0f);
    // obstacles_positions(9) = vec3(5.0f, 0.8f, -6.0f);
    // obstacles_positions(10) = vec3(5.0f, 1.0f, -6.0f);
    // obstacles_positions(11) = vec3(5.0f, 1.2f, -6.0f);
    ...
    // obstacles_scales(3) = vec3(6.0f, 0.2f, 2.0f);
    // obstacles_scales(4) = vec3(4.0f, 0.2f, 2.0f);
    // obstacles_scales(5) = vec3(2.0f, 0.2f, 2.0f);

    // obstacles_scales(6) = vec3(4.0f, 0.2f, 2.0f);
    // obstacles_scales(7) = vec3(3.5f, 0.2f, 2.0f);
    // obstacles_scales(8) = vec3(3.0f, 0.2f, 2.0f);
    // obstacles_scales(9) = vec3(2.5f, 0.2f, 2.0f);
    // obstacles_scales(10) = vec3(2.0f, 0.2f, 2.0f);
    // obstacles_scales(11) = vec3(1.5f, 0.2f, 2.0f);

    // Terrain

    ifstream file("resources/terrain_data.txt");
    if (!file.is_open()) {
    ...
    Mesh mesh = GenMeshHeightmap(image, (Vector3){ 20, (max_height - min_height), 20 }); // Generate heightmap mesh (RAM and VRAM)
    Model model = LoadModelFromMesh(mesh);                  // Load model from generated mesh
    model.materials[0].shader = ground_plane_shader;

    Vector3 mapPosition = { -10.0f, min_height, -10.0f };
    ...
    lights[0] = CreateLight(LIGHT_DIRECTIONAL, (Vector3){ 0.0f, 1.0f, 1.0f }, Vector3Zero(), WHITE, shader);
    ...
    //    if (IsKeyPressed(KEY_Q)) mode = 1;
    //    if (IsKeyPressed(KEY_W)) mode = 2;
    //    if (IsKeyPressed(KEY_E)) mode = 3;
    //    if (IsKeyPressed(KEY_SPACE)) mode = 0;
            ...
            else
            {
                ...
                if (best_index != frame_index)
                {
                    ...
                    inertialize_pose_transition(
                        ...
                        // cout << "bone x:" << bone_positions(0).x << " y:" << bone_positions(0).y << " z:" << bone_positions(0).z << endl; 
                        // cout << "offset x:" << bone_offset_positions(0).x << " y:" << bone_offset_positions(0).y << " z:" << bone_offset_positions(0).z << endl; 
        ...
        // SetShaderValue(&light);
        
        // EndTextureMode();
        // Render objects from light perspective
        // DrawModel(model, Vector3Zero(), 1.0f, WHITE); 

        EndMode3D();

        // DrawTexturePro(shadowMap.texture, (Rectangle){ 0, 0, shadowMap.texture.width, -shadowMap.texture.height }, (Rectangle){ screen_width - 128, 0, 128, 128 }, Vector2Zero(), 0.0f, WHITE);
